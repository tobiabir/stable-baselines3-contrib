...
class CategoricalNetwork(BasePolicy):
    ...
    :param v_min: the value distribution support is [v_min, v_max]. If None, we set it to be -v_max.
    :param v_max: the value distribution support is [v_min, v_max].
    ...
    def __init__(
        ...
        self.support = nn.parameter.Parameter(th.linspace(v_min, v_max, n_atoms), requires_grad=False)
        action_dim = int(self.action_space.n)  # number of actions
        categorical_net = create_mlp(self.features_dim, action_dim * self.n_atoms, self.net_arch, self.activation_fn)
        self.categorical_net = nn.Sequential(*categorical_net)

    def forward(self, obs: PyTorchObs) -> th.Tensor:
        """
        Predict the categorical logits.

        :param obs: Observation
        :return: The estimated categorical logits for each action.
        """
        logits = self.categorical_net(self.extract_features(obs, self.features_extractor))
        return logits.view(-1, int(self.action_space.n), self.n_atoms)

    def _predict(self, observation: PyTorchObs, deterministic: bool = True) -> th.Tensor:
        q_values = (th.softmax(self(observation), dim=-1) * self.support).mean(dim=-1)
        # Greedy action
...
class C51Policy(BasePolicy):
    """
    Policy class with categorical and target networks for C51
...
class CnnPolicy(C51Policy):
    """
    Policy class for C51 when using images as input.
...
class MultiInputPolicy(C51Policy):
    """
    Policy class for C51 when using dict observations as input.
